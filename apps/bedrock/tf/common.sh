...
check_prereqs() {
    if [ -z "${BEDROCK_PROVISIONING_REGION}" ]; then
    echo "BEDROCK_PROVISIONING_REGION must be set"
    exit -1
    fi
...
BEDROCK_PROVISIONING_BUCKET="bedrock-multi-region-tf-state"
STATE_BUCKET_REGION="us-west-2"

setup_tf_s3_state_store() {
    echo "Creating Terraform state bucket at s3://${BEDROCK_PROVISIONING_BUCKET} (region ${STATE_BUCKET_REGION})"
    # The following environment variables are defined in config.sh
    aws s3 mb s3://${BEDROCK_PROVISIONING_BUCKET} --region ${STATE_BUCKET_REGION}
}
...
check_state_store() {
    ...
    set +e
    if aws s3 ls s3://${BEDROCK_PROVISIONING_BUCKET} > /dev/null 2>&1; then
