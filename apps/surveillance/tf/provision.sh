...
SURVEILLANCE_PROVISIONING_REGION="us-west-2"
TERRAFORM_ENV="surveillance"
SURVEILLANCE_PROVISIONING_BUCKET="surveillance-provisioning-tf-state"
STATE_BUCKET_REGION="us-west-2"

setup_tf_s3_state_store() {
    echo "Creating Terraform state bucket at s3://${SURVEILLANCE_PROVISIONING_BUCKET} (region ${STATE_BUCKET_REGION})"
    # The following environment variables are defined in config.sh
    aws s3 mb s3://${SURVEILLANCE_PROVISIONING_BUCKET} --region ${STATE_BUCKET_REGION}
}

setup_tf_envs() {
    # this MUST be run in the dir that this file resides in
    set +e
    terraform env new surveillance
    set -e
}
...
check_state_store() {
    ...
    set +e
    if aws s3 ls s3://${SURVEILLANCE_PROVISIONING_BUCKET} > /dev/null 2>&1; then
