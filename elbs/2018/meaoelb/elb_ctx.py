...
class ELBContext:
    ...
    def get_service_nodeport(self, service_namespace, service_name):
        ...
        """
        response = self.v1.read_namespaced_service(service_name, service_namespace)
    ...
    def create_elb(self, service_config):
        ...
        if self.dry_run_mode:
            print("ELB {} would have been created".format(service_config.elb_config.name))
            return
    ...
    def attach_elbs_to_asg(self, asg_name, elb_names):
        ...
        """
        if self.dry_run_mode:
            print("The following ELBs would have been attached to the {} ASG:".format(asg_name))
            print(elb_names)
            return
    ...
    def attach_all_elbs(self, asg, services):
        ...
        """
        self.attach_elbs_to_asg(asg,
                    list(map(lambda e: e.elb_config.name, services)))